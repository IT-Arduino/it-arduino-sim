 ...
 *
 * Почему отдельный модуль, а не поле в `itArduinoAuth`: там состояние
 * появляется синхронно (токен либо есть, либо нет), а роль требует запроса
 * к сайту. Смешав их, пришлось бы каждому потребителю объяснять, почему
...
export function refreshRole(): Promise<void> {
  ...
  _inFlight = getMe()
    ...
    .catch(() => {
      // Сеть недоступна или токен протух. Молча считаем обычным
